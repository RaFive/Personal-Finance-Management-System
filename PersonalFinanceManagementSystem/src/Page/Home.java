...
public class Home extends javax.swing.JFrame {
    ...
    public Home(String userDbPath) {
        ...
        PieChartPanel.setSize(450, 300);
        PieChartPanel.setPreferredSize(new java.awt.Dimension(450, 300));
...
public JPanel createPieChart(DBK userDBK) {
    ...
    pieChart.addSeries("Uang Masuk", d.uangMasuk);
    pieChart.addSeries("Uang Keluar", d.uangKeluar);
    pieChart.addSeries("Uang Sekarang", d.uangSekarang);
    ...
    pieChart.getStyler().setSeriesColors(colors);
    ...
    pieChart.getStyler().setChartTitleFont(new java.awt.Font("SansSerif", java.awt.Font.BOLD, 18));
    pieChart.getStyler().setLegendFont(new java.awt.Font("SansSerif", java.awt.Font.PLAIN, 14));

    pieChart.getStyler().setLabelsVisible(true);
    pieChart.getStyler().setLabelsDistance(0.70);
    pieChart.getStyler().setLabelsFont(new java.awt.Font("SansSerif", java.awt.Font.BOLD, 14));
    pieChart.getStyler().setLabelType(org.knowm.xchart.style.PieStyler.LabelType.Percentage);
...
private void tampilkanChart() {
    ...
    PieChartPanel.setLayout(new BorderLayout());
    PieChartPanel.add(createPieChart(userDBK), BorderLayout.CENTER);
    ...
    private void MengubahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MengubahActionPerformed
        ...
        if (jumlahText.startsWith("+")) {
            masukLama = Integer.parseInt(jumlahText.substring(2).trim());
            jenisLama = "Uang Masuk";
        } else if (jumlahText.startsWith("-")) {
            keluarLama = Integer.parseInt(jumlahText.substring(2).trim());
            jenisLama = "Uang Keluar";
        }
        ...
        if (jenisLama.equals("Uang Keluar")) {
            boxJenis.setSelectedIndex(1);
        } else {
            boxJenis.setSelectedIndex(0);
        }

        int jumlahLama = masukLama > 0 ? masukLama : keluarLama;
        JTextField txtJumlah = new JTextField(String.valueOf(jumlahLama));
        ...
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                userDBK.updateDataById(id, masukBaru, keluarBaru, tanggal, ket);
    ...
    private void MenambahkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MenambahkanActionPerformed
        ...
        String tanggalHariIni = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
